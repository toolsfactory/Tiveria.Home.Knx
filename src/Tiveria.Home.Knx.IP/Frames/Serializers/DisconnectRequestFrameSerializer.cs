...
namespace Tiveria.Home.Knx.IP.Frames.Serializers
{
    ...
    /// </code>
    /// </summary>
    public class DisconnectRequestFrameSerializer : FrameSerializerBase<DisconnectRequestFrame>
    {
        public override ServiceTypeIdentifier ServiceTypeIdentifier => ServiceTypeIdentifier.DisconnectRequest;

        public override DisconnectRequestFrame Deserialize(BigEndianBinaryReader reader)
        {
            var header = FrameHeader.Parse(reader);
            var channelId = reader.ReadByte();
            reader.ReadByte();
            var controlEndpoint = Hpai.Parse(reader);
            return new DisconnectRequestFrame(header, channelId, controlEndpoint);
        }

        public override void Serialize(DisconnectRequestFrame frame, BigEndianBinaryWriter writer)
        {
