...
namespace Tiveria.Home.Knx.Management
{
    ...
    public interface IConnectedDeviceManagement
    {
        ...
        Task WriteMemoryBitAsync(int address, byte[] andData, byte[] xordata);

        Task<byte[]> ReadUserMemoryAsync(int address, int count);
        Task WriteUserMemoryAsync(int address, byte[] data);
        Task<byte> AuthorizeAsync(uint key);
        Task<byte> WriteKeyAsync(uint key, byte level);
