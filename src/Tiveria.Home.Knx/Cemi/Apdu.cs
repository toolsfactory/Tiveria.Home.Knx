...
namespace Tiveria.Home.Knx.Cemi
{
    ...
    /// <code>
    /// +-----------------------------------------------------------------------+
    /// |          APDU Byte 1: 6 bit TPCI and 2 bit APCI                       |
    /// +--------+--------+--------+--------+--------+--------+--------+--------+
    ...
    /// 
    /// +-----------------------------------------------------------------------+
    /// |          APDU Byte 2: 2 bit APCI and  6 bit Data or APCI              |
    /// +--------+--------+--------+--------+--------+--------+--------+--------+
    /// | bit 0  | bit 1  | bit 2  | bit 3  | bit 4  | bit 5  | bit 6  | bit 7  |
    /// +--------+--------+--------+--------+--------+--------+--------+--------+
    /// |    APCI         |          ACPI or Data                               |
    ...
    public class Apdu
    {
        ...
        /// <exception cref="KnxBufferSizeException">Thrown if buffer is less than 2 bytes</exception>
        ...
        private void ParseApci(Span<byte> buffer)
        {
            ...
            var (offset, mask) = Cemi.ApduType.GetASDUMaskAndOffset((ApduType, buffer.Length));
            Data = buffer[offset..].ToArray();
            if (Data.Length > 0)
                Data[0] &= (byte)mask;
        }
        ...
        private void ValidateDataSize()
        {
            ...
            {
                var (Mode, MinOrExact, Max) = Cemi.ApduType.GetRequiredDataDetails(ApduType);
                if ((Mode == DataMode.None) && Data.Length > 0)
                    throw new ArgumentException($"Data must empty for APCI Type {Cemi.ApduType.ToString(ApduType)}");
                if (Mode != DataMode.None && Mode != DataMode.Unknown)
                {
                    if ((Mode == DataMode.Exact) && Data.Length != MinOrExact)
                        throw new ArgumentException($"Data too small for APCI Type {Cemi.ApduType.ToString(ApduType)}. Was {Data.Length} but should be exactly {MinOrExact}");
                    if (Data.Length < MinOrExact)
                        throw new ArgumentException($"Data too small for APCI Type {Cemi.ApduType.ToString(ApduType)}. Was {Data.Length} but should be at least {MinOrExact}");
                    if ((Mode == DataMode.MinMax) && Data.Length > Max)
                        throw new ArgumentException($"Data too big for APCI Type {Cemi.ApduType.ToString(ApduType)}. Was {Data.Length} but should {Max} at maximum");
                }
            }
        }
