...
namespace Tiveria.Knx.IP
{
    ...
    internal class HeartbeatMonitor
    {
        ...
        private readonly ILogger _logger = LogFactory.GetLogger("Tiveria.Knx.IP.HeartbeatMonitor");
        #endregion
        ...
        public HeartbeatMonitor(IPEndPoint remoteEndpoint, Hpai endpointHpai, byte channelId, int intervalSeconds, int timeoutSeconds, int retries)
        {
            ...
            _retries = retries;
            _rawPacket = new KnxNetIPFrame(new ServiceTypes.ConnectionStateRequest(channelId, endpointHpai)).ToBytes();
        }
        #endregion
        ...
        public void HandleResponse(ConnectionStateResponse response)
        {
            if (response.Status == Utils.ErrorCodes.NO_ERROR)
            {
                #if DEBUG
